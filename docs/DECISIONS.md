...
> "오디오 실행 실패는 전체 쉼 세션을 중단시키지 않는다."

| # | 규칙 | 현재 구현 |
|---|---|---|
| 1 | Timer 와 Rest Session 은 계속 진행한다 | ✅ `startAudio` 실패가 `state` 를 바꾸지 않는다 |
| 2 | 오류는 `audioError` 또는 이에 준하는 상태로 기록한다 | ✅ `RestFlowCoordinator.audioError` |
| 3 | **사용자에게 기술 오류 메시지를 크게 노출하지 않는다** | ✅ 어떤 View 도 `audioError` 를 그리지 않는다 |
| 4 | 현재 Sprint 에서 자동 대체 오디오를 구현하지 않는다 | ✅ 미구현 |
| 5 | 향후 Rest Engine 에서 fallback 가능한 구조를 고려한다 | 📌 **B-009** |

### 근거 (Product Owner)

> "음악 파일 하나가 실패했다고 사용자가 이미 시작한 10분 쉼 자체를 종료해버리는 게
> 제품 경험상 더 나쁘기 때문이야."

제품 기준서와도 맞는다. 소리가 없어도 **"10분 동안 화면에서 벗어난다"** 는
핵심 경험은 성립한다. 오디오는 환경을 돕는 요소이지 쉼 자체가 아니다.

### 규칙 3 을 코드로 지키는 방법

`audioError` 는 **진단용 상태이지 화면에 그릴 문구가 아니다.**
`AudioServiceError.description` 은 "음원 'test_ambient' 을 찾을 수 없습니다" 같은
기술적 문장이라 그대로 노출하면 안 된다.

RestSession 화면은 "남은 시간 + 한 문장 안내 + 중단 버튼" 뿐이어야 한다
(`docs/IOS_SPEC.md` §8.2). 오류 문구를 넣는 것 자체가 제품 원칙과 부딪힌다.

`scripts/verify_repo.py` 가 **View 파일에서 `audioError` 참조를 차단**한다.
나중에 누군가 무심코 화면에 바인딩하는 것을 막기 위해서다.
...
Rest Engine 확인
   ↓
대체 가능 → SILENCE / NATURE_SOUND / no-audio 로 교체
대체 불가 → 세션은 계속 + 사용자에게 최소 안내
```

`docs/PRODUCT.md` §6 의 Rest Blocks 구조와 이어진다.
Rest Engine 이 생기는 Sprint 에서 다시 다룬다. **지금은 구현하지 않는다.**
