...
| 2 | **background 지속 재생** | 재생 중 홈 버튼/제스처로 홈 화면 이동 | **소리가 끊기지 않는다** | ⬜ |
| 3 | **화면 잠금 지속 재생** | 전원 버튼으로 화면 잠금 | **소리가 계속 난다** | ⬜ |
| 4 | **앱 복귀 후 상태 정상** | 잠금 해제 후 앱으로 복귀 | 남은 시간이 실제 경과분만큼 줄어 있다. 소리는 계속 난다 | ⬜ |
| 5 | **정상 종료 시 즉시 정지** | 남은 시간이 0이 될 때까지 대기 | 결과 화면으로 넘어가며 **소리가 즉시 멈춘다** | ⬜ |
| 6 | **취소 시 즉시 정지** | 재생 중 "쉼 그만하기" | **소리가 즉시 멈춘다** | ⬜ |
...
1. 위 표에서 통과/실패한 항목 번호
2. 실패한 항목의 실제 동작 (무엇이 기대와 달랐는지)
3. iPhone 모델과 iOS 버전
4. 10번 항목의 관찰 결과

**모두 통과하면 Sprint 3 을 DONE 으로 처리한다.**
